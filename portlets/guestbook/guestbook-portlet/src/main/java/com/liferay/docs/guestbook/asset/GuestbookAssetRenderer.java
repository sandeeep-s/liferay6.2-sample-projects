...
public class GuestbookAssetRenderer extends BaseAssetRenderer {
	...
	public boolean hasEditPermission(PermissionChecker permissionChecker) {
		...
		try {
			contains = GuestbookPermission.contains(permissionChecker, guestbookId, ActionKeys.UPDATE);
	...
	public boolean hasViewPermission(PermissionChecker permissionChecker) {
		...
		try {
			contains = GuestbookPermission.contains(permissionChecker, guestbookId, ActionKeys.VIEW);
		} catch (PortalException pe) {
			_log.error(pe.getLocalizedMessage());
		} catch (SystemException se) {
			_log.error(se.getLocalizedMessage());
