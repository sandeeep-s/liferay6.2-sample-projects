...
<aui:button-row cssClass="guestbook-buttons">
	...
	<c:if test='<%=GuestbookModelPermission.contains(permissionChecker, scopeGroupId, "ADD_GUESTBOOK")%>'>
		<portlet:renderURL var="addGuestbookURL">
			<portlet:param name="action" value="viewGuestbookEditForm" />
		</portlet:renderURL>
	...
	<c:if test='<%=GuestbookPermission.contains(permissionChecker, guestbook.getGuestbookId(), "ADD_ENTRY")%>'>
		<portlet:renderURL var="addEntryURL">
			<portlet:param name="action" value="viewEntryEditForm" />
...
<liferay-ui:search-container>
	<liferay-ui:search-container-results
		...
		total="<%=EntryLocalServiceUtil.getEntriesCount(scopeGroupId, guestbook.getGuestbookId())%>" />
